...
DevAbs:del(SNAYBIR.."Abs:Lock:AutoFile")
end
...
DevAbs:set(SNAYBIR.."Abs:Lock:AutoFile",true) 
end 
end

if (text and not DevAbs:get(SNAYBIR.."Abs:Lock:AutoFile")) then
Time = DevAbs:get(SNAYBIR.."Abs:AutoFile:Time")
if Time then 
...
BasicConstructors = DevAbs:smembers(SNAYBIR..'Abs:BasicConstructor:'..v)
Constructors = DevAbs:smembers(SNAYBIR..'Abs:Constructor:'..v)
...
local curl = 'curl "' .. abbas .. '" -F "chat_id='..DevId..'" -F "document=@'..SNAYBIR..'.json' .. '" -F "caption=⌁︙يحتوي الملف على ↫ '..#list..' مجموعه"'
io.popen(curl)
io.popen('fm -fr '..SNAYBIR..'.json')
DevAbs:set(SNAYBIR.."Abs:AutoFile:Time",os.date("%x"))
end
else 
DevAbs:set(SNAYBIR.."Abs:AutoFile:Time",os.date("%x"))
end
